...
const GeneralTools: React.FC<ToolProps> = ({ slug, onSuccess, onError }) => {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      setOutputVal(result);
      if (slug !== 'password-generator' || inputVal) {
    ...
    <>
      ...
      {(slug === 'password-generator' || slug === 'lorem-ipsum-generator') && (
        <div className="py-20 text-center flex flex-col items-center">
          ...
          <p className="text-slate-400 font-medium">Automatic generation mode active.</p>
        </div>
      )}
    </>
  );
  ...
  const actionsSlot = (
    <div className="flex flex-col sm:flex-row gap-4 w-full">
      ...
      >
        {loading ? "Processing..." : slug === 'password-generator' ? "Generate New Password" : "Run Processing"}
      </button>
      {(outputVal || qrUrl) && (
  ...
  return (
    <ToolLayout
      ...
      description={`Free online ${slug.replace(/-/g, ' ')} for professional developers and creators.`}
      icon="🛠️"
      input={inputSlot}
      actions={actionsSlot}
